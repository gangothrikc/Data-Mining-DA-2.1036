...
dist_matrix <- dist(data[, 2:7])  # Calculate the distance matrix
hclust_result <- hclust(dist_matrix, method = "complete")  # Hierarchical clustering using complete linkage method

# Plot the dendrogram
plot(hclust_result, main = "Hierarchical Clustering Dendrogram", xlab = "Universities", sub = "")
